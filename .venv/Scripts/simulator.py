...
import numpy as np
import util
from NN_replacement import nn_policy
...
def lazy_update(node_address, pc):
    ...
    if level == TREE_LEVELS - 1:
        ...
        cache = LLC_ctr_3
    cache_block = cache.read(parent_address, pc, level=level)
    if cache_block:
        cache.write(parent_address, byte, pc, level=level)
        hit = True
    else:
        victim_address = cache.load(parent_address, data=bytearray(8), pc=pc, level=level, lazy_update=1)
        cache.write(parent_address, byte, pc, level=level)
...
class MemoryAccess:
    def __init__(self, access_type, address, byte=00, pc=0):
        ...
        self.byte = byte
        self._pc = pc
        address_diff = abs(np.int64(address) - np.int64(Parameters.previous_address))
        ...
        Parameters.previous_address = address
        global cache_hit
        if access_type == "read":
...
def benchmark_trace(MAX_INSTRUCTIONS):
    mm = MemoryManager()
    Parameters.instructions_number = 0
    ...
    for x in range(MAX_INSTRUCTIONS):
        ...
        if Parameters.instructions_number == 4700000:
...
for E in range(1):
    nn_policy.epsilon *= nn_policy.epsilon_decay
    for k in range(len(simulations)):
        ...
        LLC_ctr_0_size = (simulations[k][1] // 2)
        LLC_ctr_0 = Cache(LLC_ctr_0_size, simulations[k][0], simulations[k][2],
                          2 ** 3, replace_pol="ship_plus", write_pol=simulations[k][5], type="ctr_cache0")
        LLC_ctr_1_size = (simulations[k][1] // 256)
        LLC_ctr_1 = Cache(LLC_ctr_0_size, simulations[k][0], simulations[k][2],
                          2 ** 2, replace_pol="ship_plus", write_pol=simulations[k][5], type="ctr_cache1")
        LLC_ctr_2 = Cache(simulations[k][1] // 256, simulations[k][0], simulations[k][2],
                          2 ** 2, replace_pol="ship_plus", write_pol=simulations[k][5], type="ctr_cache2")
        LLC_ctr_3 = Cache(simulations[k][1] // 256, simulations[k][0], simulations[k][2],
                          2 ** 2, replace_pol="ship_plus", write_pol=simulations[k][5], type="ctr_cache3")
        ...
        if nn_policy.evaluate_mode == True:
            nn_policy.store([0] * 100, 0, 0, [0] * 100, True)
            with open("Cache_Misses.txt", "a") as f:
                f.write(f"Episode {E}: Counter level misses in LLC counter cache = {LLC_ctr_level_misses}\n")
            LLC_ctr_0.save_weights()
        ...
        with open("Optimize_level_size.txt", "a") as f:
            f.write(f"Sim {E}, Sets LLC0:{LLC_ctr_0_size // (8 * 64)}, Sets LLC 1:{LLC_ctr_1_size // (4 * 64)}\n")
            f.write(f"LLC counter cache hitrate = {(ctr_cache_hits / (ctr_cache_hits + ctr_cache_misses))}\n")
            f.write(f"Counter level hits in LLC counter cache = {LLC_ctr_level_hits}\n")
            f.write(f"Counter level misses in LLC counter cache = {LLC_ctr_level_misses}\n")
            f.write(f"Total counter LLC misses = {ctr_cache_misses}\n")

# LLC_ctr_0.weight_contributions()
